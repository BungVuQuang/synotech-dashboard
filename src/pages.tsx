...
export function LoginPage(){
  ...
  return <div className="login-page">
    <div className="login-visual">
      <div className="visual-badge">Synotech</div>
      <h1>Quản lý công việc, khách hàng và dịch vụ tại một nơi.</h1>
      <p>Trang quản trị hỗ trợ đội ngũ theo dõi hoạt động hằng ngày, xử lý yêu cầu và cập nhật thông tin thuận tiện hơn.</p>
      <div className="visual-points"><span><CheckCircle2/>Theo dõi hoạt động</span><span><CheckCircle2/>Quản lý khách hàng</span><span><CheckCircle2/>Báo cáo rõ ràng</span></div>
    </div>
    <form className="login-card" onSubmit={submit}>
      <div className="brand brand-login"><span className="brand-mark">S</span><div><strong>Synotech</strong><small>Giải pháp công nghệ</small></div></div>
...
export function KnowledgeBasePage(){const {clientId,withClient}=useOutletContext<DashboardOutletContext>();const [tab,setTab]=useState('documents');const [group,setGroup]=useState('');const [selected,setSelected]=useState<KbDocument|null>(null);const docs=useLoad(()=>api<ApiList<KbDocument>>(withClient(`/v1/admin/kb/documents?limit=500${group?`&knowledge_group=${group}`:''}`)),[clientId,group]);const datasets=useLoad(()=>api<ApiList<any>>(withClient('/v1/admin/kb/datasets')),[clientId]);const sync=async()=>{await api(withClient('/v1/admin/kb/sync'),{method:'POST',body:JSON.stringify({})});docs.reload();datasets.reload()};return <><PageHeader title="Kho tri thức" description="Đồng bộ dữ liệu, kiểm tra thông tin mô tả và quản lý trạng thái tài liệu." actions={<button className="btn btn-primary" onClick={sync}><RefreshCw size={16}/>Đồng bộ ngay</button>}/><Tabs active={tab} onChange={setTab} items={[{key:'documents',label:'Tài liệu'},{key:'datasets',label:'Nguồn dữ liệu'},{key:'validation',label:'Kiểm tra thông tin'},{key:'deleted',label:'Đã xóa'}]}/>{tab==='datasets'?<SectionCard title="Nguồn dữ liệu đã kết nối">{datasets.loading?<LoadingState/>:<DataTable rows={datasets.data?.results||[]} keyOf={(r:any)=>r.id} columns={[{key:'name',title:'Nguồn dữ liệu',render:(r:any)=><div><strong>{r.dataset_name}</strong><small className="table-sub mono">{r.dify_dataset_id}</small></div>},{key:'sync',title:'Đồng bộ',render:(r:any)=><StatusBadge value={r.last_sync_status||'pending'}/>},{key:'time',title:'Lần cuối',render:(r:any)=>formatDate(r.last_synced_at)},{key:'interval',title:'Chu kỳ',render:(r:any)=>`${r.sync_interval_minutes} phút`}]}/>}</SectionCard>:<><Toolbar><select value={group} onChange={e=>setGroup(e.target.value)}><option value="">Tất cả nhóm nội dung</option>{['admission','tuition','major','academic','exam','graduation','clinical','online_learning','student_policy','campus','contact','legal_policy'].map(x=><option key={x} value={x}>{knowledgeGroupLabel(x)}</option>)}</select></Toolbar>{docs.loading?<LoadingState/>:docs.error?<ErrorState message={docs.error} retry={docs.reload}/>:<DataTable rows={(docs.data?.results||[]).filter(r=>tab==='deleted'?r.sync_status==='deleted':tab==='validation'?r.metadata_check_status!=='valid':r.sync_status!=='deleted')} keyOf={r=>r.id} onRowClick={setSelected} columns={[{key:'doc',title:'Tài liệu',render:r=><div><strong>{shortText(r.document_name,60)}</strong><small className="table-sub mono">{r.source_id||'Thiếu source_id'}</small></div>},{key:'group',title:'Nhóm nội dung',render:r=><span className="tag">{r.knowledge_group||'—'}</span>},{key:'year',title:'Năm/Đợt',render:r=>r.year?`${r.year}${r.round_no!=null?` · Đợt ${r.round_no}`:''}`:'—'},{key:'status',title:'Trạng thái tài liệu',render:r=><StatusBadge value={r.actual_status}/>},{key:'index',title:'Lập chỉ mục',render:r=><StatusBadge value={r.indexing_status}/>},{key:'metadata',title:'Thông tin mô tả',render:r=><StatusBadge value={r.metadata_check_status}/>},{key:'sync',title:'Đồng bộ',render:r=><StatusBadge value={r.sync_status}/>} ]}/>}</>}<Drawer open={!!selected} title={selected?.document_name||''} onClose={()=>setSelected(null)}><KbDocumentEditor document={selected} onSaved={docs.reload}/></Drawer></>}

function KbDocumentEditor({document,onSaved}:{document:KbDocument|null;onSaved:()=>void}){const [metadata,setMetadata]=useState({client_id:document?.client_id||'',knowledge_group:document?.knowledge_group||'',source_id:document?.source_id||'',source_url:document?.source_url||'',year:document?.year||'',round_no:document?.round_no??''});if(!document)return null;const save=async()=>{await api(`/v1/admin/kb/documents/${document.id}/metadata?client_id=${document.client_id}`,{method:'PATCH',body:JSON.stringify({metadata})});onSaved()};const status=async(action:string)=>{await api(`/v1/admin/kb/documents/${document.id}/status?client_id=${document.client_id}`,{method:'PATCH',body:JSON.stringify({action})});onSaved()};return <div className="form-grid"><label>Mã khách hàng<input value={metadata.client_id} onChange={e=>setMetadata({...metadata,client_id:e.target.value})}/></label><label>Nhóm nội dung<input value={metadata.knowledge_group} onChange={e=>setMetadata({...metadata,knowledge_group:e.target.value})}/></label><label>Mã nguồn<input value={metadata.source_id} onChange={e=>setMetadata({...metadata,source_id:e.target.value})}/></label><label>Đường dẫn nguồn<input value={metadata.source_url} onChange={e=>setMetadata({...metadata,source_url:e.target.value})}/></label><label>Năm<input type="number" value={metadata.year} onChange={e=>setMetadata({...metadata,year:e.target.value})}/></label><label>Đợt tuyển sinh<input type="number" value={metadata.round_no} onChange={e=>setMetadata({...metadata,round_no:e.target.value})}/></label><div className="full button-row"><button className="btn btn-primary" onClick={save}>Lưu thông tin</button><button className="btn btn-secondary" onClick={()=>status('enable')}>Bật</button><button className="btn btn-secondary" onClick={()=>status('disable')}>Tắt</button><button className="btn btn-secondary" onClick={()=>status('archive')}>Lưu trữ</button></div></div>}
...
export function ChatbotOperationsPage(){
  ...
  return <>
    ...
    <ChatRuntimeEditor key={clientId} clientId={clientId} data={data} secret={details.data?.secret} onSaved={details.reload}/>
...
function ChatRuntimeEditor({clientId,data,secret,onSaved}:{clientId:string;data:any;secret:any;onSaved:()=>void}){
  const runtime=data.runtime||{};
  ...
  return <SectionCard title="Kết nối và trạng thái chatbot" description="Mỗi khách hàng có cấu hình riêng. Khóa truy cập được mã hóa và không hiển thị lại sau khi lưu.">
    <div className="form-grid runtime-form">
      ...
      <label className="full">Khóa truy cập ứng dụng Dify<input type="password" value={apiKey} onChange={e=>setApiKey(e.target.value)} placeholder={secret?.configured?'Đã có khóa truy cập — chỉ nhập khi muốn thay đổi':'Nhập khóa truy cập của ứng dụng Dify'}/><small className="field-hint">Trạng thái: {secret?.configured?'Đã cấu hình':'Chưa cấu hình'}</small></label>
      ...
      <div className="button-row full"><button className="btn btn-primary" disabled={saving} onClick={save}><Save size={16}/>Lưu cấu hình</button><button className="btn btn-secondary" disabled={saving} onClick={test}><RefreshCw size={16}/>Kiểm tra kết nối</button><button className="btn btn-secondary" disabled={saving} onClick={activate}><Bot size={16}/>Kích hoạt chatbot</button>{secret?.configured&&<button className="btn btn-danger" disabled={saving} onClick={removeKey}><Trash2 size={16}/>Xóa khóa truy cập</button>}</div>
    </div>
  </SectionCard>;
}
...
export function AnalyticsPage(){
  ...
  <div className="two-column"><SectionCard title="Chất lượng xử lý câu hỏi nhiều nội dung"><DataTable rows={data?.tasks||[]} keyOf={(r:any)=>`${r.knowledge_group}-${r.status}`} columns={[{key:'group',title:'Nhóm nội dung',render:(r:any)=><span className="tag">{knowledgeGroupLabel(r.knowledge_group)}</span>},{key:'status',title:'Trạng thái',render:(r:any)=><StatusBadge value={r.status}/>},{key:'count',title:'Số tác vụ',render:(r:any)=>r.task_count},{key:'score',title:'Điểm khớp trung bình',render:(r:any)=>r.avg_max_score??'—'},{key:'latency',title:'Thời gian xử lý',render:(r:any)=>r.avg_latency_ms?`${Math.round(r.avg_latency_ms)} mili giây`:'—'}]}/></SectionCard><SectionCard title="Chất lượng phân loại câu hỏi"><DataTable rows={data?.quality||[]} keyOf={(r:any)=>`${r.classification_version}-${r.knowledge_group}`} columns={[{key:'group',title:'Nhóm nội dung',render:(r:any)=><span className="tag">{knowledgeGroupLabel(r.knowledge_group)}</span>},{key:'version',title:'Phiên bản phân loại',render:(r:any)=>r.classification_version},{key:'count',title:'Số câu',render:(r:any)=>r.classified_count},{key:'confidence',title:'Độ tin cậy',render:(r:any)=>`${Math.round(Number(r.avg_confidence||0)*100)}%`},{key:'low',title:'Độ tin cậy thấp',render:(r:any)=>r.low_confidence_count}]}/></SectionCard></div>
